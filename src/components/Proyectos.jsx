...
export default function Proyectos({ proyectos = [], onOpenProject, colors }) {
  return (
    <section
      id="mis-proyectos"
      className="min-h-screen px-6 md:px-20 py-20 flex flex-col items-center"
      style={{ backgroundColor: colors.fondoP, color: colors.textWh }}
    >
      <h2 className="text-3xl font-light mb-12 text-center">Mis Proyectos</h2>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-6xl">
        {proyectos.map((p) => (
          <div key={p.id} className="group relative overflow-hidden rounded-none shadow-lg">
            <button
      ...
      <div className="flex justify-center mt-12">
       <ArrowDown
  href="#mis-renders-fotorealistas-y-post-producción"
  forceColor="dark"
/>
      </div>
