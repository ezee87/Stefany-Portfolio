...
export default function DiagramasGallery({ diagramas = [], onImageClick = () => {}, colors }) {
  const defaults = [
  ...
  return (
    <section
      ...
      style={{ backgroundColor: colors.fondoA, color: colors.text }}
    >
      {/* Desktop grid */}
      <div
      ...
      >
        {/* Columna del título */}
        <div
          className="relative flex flex-col justify-start items-center h-full box-border"
          style={{ paddingTop: "25%" }}
        >
          ...
          <div className="mt-auto mb-6 flex justify-center w-full">
            <ArrowDown href="#contacto" dark={true} />
          </div>
        </div>
        ...
        {items.map((d, idx) => {
          ...
          const colorBlock = (
            ...
            >
              <p
                className={`text-xl font-medium ${paletteItem.textClass} text-center break-words px-2`}
              >
                {d.title}
              </p>
            </div>
      ...
      <div className="md:hidden w-full h-full overflow-y-auto">
        <h2 className="text-3xl font-light py-6 text-center" style={{ color: colors.text }}>
        ...
        <div className="flex flex-col space-y-4 px-2">
          {items.map((d, idx) => (
            <div
              key={idx}
              className="w-full flex flex-col h-auto rounded-2xl overflow-hidden shadow-sm"
            >
              <button
                onClick={() => onImageClick(d.src)}
